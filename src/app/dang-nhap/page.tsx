...
export default function LoginPage() {
  ...
  const reauth = typeof window !== "undefined" && new URLSearchParams(window.location.search).get("reauth") === "1";
    ...
    fetch(`/api/auth/me?next=${encodeURIComponent(next)}`, { cache: "no-store" })
      ...
      .then((result) => {
        ...
        if (result.user && result.profile?.is_active !== false) {
          if (result.managementRequired) {
            setManagementRequired(true);
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true);
    const response = await fetch("/api/auth/login", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ phone, password, next, reauth: managementRequired }) });
