...
const profileSchema = z.object({ fullName: z.string().trim().min(2).max(100), phone: normalizedPhoneSchema });
...
export async function PATCH(request: Request) {
  ...
  try {
    const supabase = createSupabaseAdminClient();
    const { data, error } = await supabase.from("profiles").update({ full_name: parsed.data.fullName, phone: parsed.data.phone, updated_at: new Date().toISOString() }).eq("id", user.id).select("id, full_name, phone, role, is_active").single();
