...
const transactionSchema = z.object({
  transactionType: z.enum(["import", "export", "damaged", "adjustment"]),
...
export async function GET(_request: Request, { params }: RouteContext) {
  ...
  if (!idSchema.safeParse(id).success) return errorResponse("INVALID_INVENTORY_ID", "Item kho không hợp lệ.", 400);
  try {
    const supabase = createSupabaseAdminClient();
    const [itemResult, transactionsResult] = await Promise.all([
      supabase.from("inventory_items").select("id, name, unit, inventory_type, inventory_unit_id, quantity_on_hand, quantity_reserved, low_stock_threshold, is_active, created_at, updated_at").eq("id", id).single(),
      supabase.from("inventory_transactions").select("id, inventory_item_id, transaction_type, quantity_change, quantity_before, quantity_after, reason, note, order_id, created_at, created_by").eq("inventory_item_id", id).order("created_at", { ascending: false }).limit(100),
    ]);
    if (itemResult.error || !itemResult.data) return errorResponse("INVENTORY_NOT_FOUND", "Không tìm thấy item trong kho.", 404);
    ...
    return errorResponse("INVENTORY_SERVICE_UNAVAILABLE", "Dịch vụ lịch sử kho tạm thời không khả dụng.", 503);
  }
}
...
export async function POST(request: Request, { params }: RouteContext) {
  ...
  try {
    ...
    if (error) {
      ...
      if (error.message.includes("inventory_item_not_found")) return errorResponse("INVENTORY_NOT_FOUND", "Không tìm thấy item trong kho.", 404);
      if (error.message.includes("import_must_increase_stock") || error.message.includes("stock_out_must_decrease_stock")) return errorResponse("INVENTORY_TRANSACTION_DIRECTION_INVALID", "Chiều tăng/giảm không phù hợp với loại giao dịch.", 400);
    ...
    return errorResponse("INVENTORY_SERVICE_UNAVAILABLE", "Dịch vụ giao dịch kho tạm thời không khả dụng.", 503);
  }
}
