...
import ProductImageCropEditor from "@/components/admin/ProductImageCropEditor";
import type { ProductImageCrop } from "@/lib/products";

type Status = "draft" | "published" | "hidden" | "archived";
type ProductType = "bouquet" | "basket";
type ImageItem = { id: string; storage_path: string; public_url: string; alt_text: string; display_order: number; is_cover: boolean; mime_type?: string; crop_x?: number | null; crop_y?: number | null; crop_zoom?: number | null; focal_x?: number | null; focal_y?: number | null };
...
const adminImageStyle = (image: Pick<ImageItem, "focal_x" | "focal_y" | "crop_zoom">) => { const focalX = Number.isFinite(Number(image.focal_x)) ? Number(image.focal_x) : 0.5; const focalY = Number.isFinite(Number(image.focal_y)) ? Number(image.focal_y) : 0.5; const zoom = Number.isFinite(Number(image.crop_zoom)) ? Number(image.crop_zoom) : 1; return { objectPosition: `${focalX * 100}% ${focalY * 100}%`, transform: `scale(${zoom})`, transformOrigin: `${focalX * 100}% ${focalY * 100}%` }; };
...
export default function AdminProductsPage() {
  ...
  const closeEditor = () => { setShowEditor(false); setSelected(null); setCropImage(null); setError(""); setNotice(""); };
  ...
  function openCropEditor(image: ImageItem) { setCropImage(image); setError(""); setNotice(""); }

  async function saveImageCrop(crop: ProductImageCrop) {
    if (!cropImage) return;
    const saved = await patchImage(cropImage, crop);
    if (!saved) return;
    setCropImage(null);
    setNotice("Đã lưu vùng hiển thị 2:3.");
  }

  async function patchImage(image: ImageItem, values: Record<string, unknown>): Promise<boolean> {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) { const message = apiErrorMessage(result, "Không thể cập nhật ảnh."); setError(message); toast.error(message); return false; }
  ...
  return <main className="min-h-screen bg-background px-4 py-5 sm:px-8 sm:py-7"><div className="mx-auto max-w-7xl"><section className="mt-6 rounded-[24px] border border-border bg-surface p-4 sm:p-7"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.18em] text-primary">Catalog control</p><h1 className="mt-2 font-display text-4xl sm:text-5xl">Sản phẩm</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Quản lý thông tin, hình ảnh, trạng thái hiển thị và lịch sử sản phẩm. Dữ liệu lưu trong PostgreSQL và ảnh trong Supabase Storage.</p></div><button onClick={openNew} className="inline-flex min-h-11 items-center gap-2 rounded-full bg-primary px-5 py-3 text-sm font-bold text-white"><Plus size={17} /> Thêm sản phẩm</button></div>
    ...
    {cropImage && selected && <ProductImageCropEditor image={{ src: cropImage.public_url, alt: cropImage.alt_text || selected.name, cropX: cropImage.crop_x ?? 0.5, cropY: cropImage.crop_y ?? 0.5, cropZoom: cropImage.crop_zoom ?? 1, focalX: cropImage.focal_x ?? 0.5, focalY: cropImage.focal_y ?? 0.5 }} productName={selected.name} onSave={saveImageCrop} onClose={() => setCropImage(null)} />}
  </main>;
}
