...
const getApiErrorMessage = (body: ApiErrorBody, fallback: string) => typeof body.error === "string" ? body.error : body.error?.message || fallback;
...
export default function InventoryPage() {
  ...
  const loadItems = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(getApiErrorMessage(result, "Không thể tải kho hoa."));
  ...
  const loadHistory = async (id: string) => {
    ...
    const result = await response.json();
    if (!response.ok) {
      setError(getApiErrorMessage(result, "Không thể tải lịch sử kho."));
      return;
    }
  ...
  const submitItem = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setError(getApiErrorMessage(result, "Không thể lưu nguyên liệu."));
        return;
      }
      ...
      setError("Không thể kết nối tới dịch vụ kho hoa. Dữ liệu chưa được lưu.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const deleteItem = async (item: InventoryItem) => {
    if (!window.confirm(`Xóa nguyên liệu “${item.name}”? Chỉ nguyên liệu chưa có công thức hoặc lịch sử tồn kho mới được xóa.`)) return;
    setMessage("");
    setError("");
    setSubmitting(true);
    try {
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) {
        setError(getApiErrorMessage(result, "Không thể xóa nguyên liệu."));
        return;
      }
      ...
      setError("Không thể kết nối tới dịch vụ kho hoa. Nguyên liệu chưa bị xóa.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const submitTransaction = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const result = await response.json().catch(() => ({}));
    if (!response.ok) {
      setError(getApiErrorMessage(result, "Không thể ghi nhận giao dịch."));
      return;
    }
