...
export default function AccountPage() {
  ...
  useEffect(() => {
    Promise.all([fetch("/api/auth/me"), fetch("/api/account/orders")]).then(async ([meResponse, ordersResponse]) => {
      ...
      setProfile(me.profile); setFullName(me.profile.full_name || ""); setPhone(me.profile.phone || "");
  ...
  async function saveProfile() {
    ...
    const response = await fetch("/api/account/profile", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ fullName, phone }) });
