...
export default function Home() {
  ...
  const visibleProducts = (() => {
    ...
    const filtered = activeProducts.filter((product) => {
      ...
      return matchesQuery && matchesType && matchesCategory && matchesTone && matchesOccasion && getPrice(product) <= selectedMaxPrice;
    });
...
function FilterBlock({ filters, setFilters, productTypes, categories, tones, occasions, priceMax, priceStep }: { filters: Filters; setFilters: (filters: Filters) => void; productTypes: ProductTypeOption[]; categories: string[]; tones: string[]; occasions: string[]; priceMax: number; priceStep: number }) {
  const maxPrice = Math.max(priceStep, priceMax);
  const value = Math.min(filters.maxPrice || maxPrice, maxPrice);
  return <div className="space-y-5"><div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Loại hoa</p><div className="flex flex-wrap gap-2 md:block md:space-y-1.5"><FilterPill active={filters.type === "all"} label="Tất cả" onClick={() => setFilters({ ...filters, type: "all" })} />{productTypes.map((type) => <FilterPill key={type.id} active={filters.type === type.slug} label={type.name} onClick={() => setFilters({ ...filters, type: type.slug })} />)}</div></div>{categories.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Danh mục</p><select value={filters.category} onChange={(event) => setFilters({ ...filters, category: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả danh mục</option>{categories.map((category) => <option key={category}>{category}</option>)}</select></div>}{tones.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Tone màu</p><select value={filters.tone} onChange={(event) => setFilters({ ...filters, tone: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả màu</option>{tones.map((tone) => <option key={tone}>{tone}</option>)}</select></div>}{occasions.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Dịp tặng</p><select value={filters.occasion} onChange={(event) => setFilters({ ...filters, occasion: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả dịp</option>{occasions.map((occasion) => <option key={occasion}>{occasion}</option>)}</select></div>}<div><div className="mb-2.5 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Giá tối đa</p><span className="text-xs font-bold text-primary">{formatVnd(value)}</span></div><input aria-label="Giá tối đa" type="range" min={priceStep} max={maxPrice} step={priceStep} value={value} onChange={(event) => setFilters({ ...filters, maxPrice: Number(event.target.value) })} className="w-full accent-[#71866c]" /></div></div>;
}
