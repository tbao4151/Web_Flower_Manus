...
const instagramProfile = "https://www.instagram.com/nfishtt_flower/";
const zaloUrl = "https://zalo.me/0356925367";
...
export default function Home() {
  ...
  const [checkout, setCheckout] = useState<CheckoutForm>({ customerName: "", customerPhone: "", recipientName: "", recipientPhone: "", address: "", deliveryDate: todayPlusOne(), deliveryTime: "", cardMessage: "", note: "" });
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      <div className="border-b border-border bg-foreground px-4 py-2 text-center text-[10px] font-bold uppercase tracking-[.16em] text-[#f6eee1] sm:text-[11px]">Có sẵn thiệp và túi · Đặt hoa qua Instagram hoặc Zalo</div>
      ...
      {checkoutOpen && <CheckoutModal checkout={checkout} updateCheckout={updateCheckout} cart={cart} total={total} submitState={submitState} errorMessage={errorMessage} orderResult={orderResult} onClose={() => { setCheckoutOpen(false); setSubmitState("idle"); }} onSubmit={createOrder} onCopy={copySummary} />}
...
function ProductCard({ product, onSelect, onAdd }: { product: Product; onSelect: () => void; onAdd: () => void }) {
  return <article className="group min-w-0"><button onClick={onSelect} className="block w-full text-left"><div className="relative overflow-hidden rounded-[18px] bg-surface-muted"><img src={product.image} alt={product.name} className="aspect-square w-full object-cover transition duration-500 group-hover:scale-[1.03]" />{product.featured && <span className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-1 text-[9px] font-bold uppercase tracking-[.12em] text-primary">Mới</span>}</div><div className="mt-2.5 min-w-0"><div className="flex items-start justify-between gap-1.5"><h3 className="line-clamp-2 min-w-0 font-display text-[17px] leading-tight sm:text-xl">{product.name}</h3><span className="mt-0.5 shrink-0 text-[10px] text-muted-foreground">{product.type === "bouquet" ? "Bó" : "Giỏ"}</span></div><div className="mt-1 flex items-center gap-1.5"><span className="text-sm font-bold text-primary sm:text-base">{formatVnd(getPrice(product))}</span>{product.salePrice && <span className="text-xs text-muted-foreground line-through">{formatVnd(product.price)}</span>}</div>{product.description && <p className="mt-1 line-clamp-1 text-[11px] leading-5 text-muted-foreground sm:text-xs">{product.description}</p>}</div></button><button onClick={onAdd} className="press mt-2 flex min-h-9 w-full items-center justify-center gap-1.5 rounded-full border border-border text-xs font-bold text-primary transition hover:border-primary hover:bg-[#e4ecdf] sm:min-h-10 sm:text-sm"><Plus size={14} /> Thêm nhanh</button></article>;
}
