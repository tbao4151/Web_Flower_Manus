...
export default function Home() {
  ...
  const [checkout, setCheckout] = useState<CheckoutForm>({ recipientName: "", recipientPhone: "", address: "", deliveryDate: todayPlusOne(), deliveryTime: "", cardMessage: "", note: "" });
    ...
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <header className="sticky top-0 z-30 border-b border-border/80 bg-background/95 backdrop-blur-xl">
        <div className="container-cas flex h-[68px] items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-1.5"><button className="flex h-10 w-10 items-center justify-center rounded-full text-foreground hover:bg-surface-muted" aria-label="Tìm kiếm sản phẩm" onClick={() => goCatalog()}><Search size={18} /></button><a href="/tai-khoan" className="hidden h-10 items-center justify-center rounded-full px-3 text-sm font-bold text-primary hover:bg-surface-muted sm:flex">Tài khoản</a><a href="/tra-cuu-don-hang" className="hidden h-10 items-center justify-center rounded-full px-3 text-sm font-bold text-muted-foreground hover:bg-surface-muted md:flex">Tra cứu đơn</a><button onClick={() => setCartOpen(true)} className="relative flex h-10 w-10 items-center justify-center rounded-full bg-surface text-foreground shadow-sm hover:bg-surface-muted" aria-label={`Giỏ hàng, ${cartCount} sản phẩm`}><ShoppingBag size={18} />{cartCount > 0 && <span className="absolute -right-0.5 -top-0.5 flex h-5 min-w-5 items-center justify-center rounded-full bg-accent px-1 text-[10px] font-bold text-white">{cartCount}</span>}</button><button onClick={() => setMobileNavOpen((open) => !open)} className="flex h-10 w-10 items-center justify-center rounded-full text-foreground hover:bg-surface-muted md:hidden" aria-label={mobileNavOpen ? "Đóng menu" : "Mở menu"} aria-expanded={mobileNavOpen}><Menu size={19} /></button></div>
        </div>
