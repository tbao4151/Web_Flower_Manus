...
export default function Home() {
  ...
  const allCategories = Array.from(new Set(catalogProducts.flatMap((product) => product.categories))).sort();
  ...
  const createOrder = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (requiredPreorderHours > 0) {
      ...
      if (!receiveAt || receiveAt.getTime() - Date.now() < requiredPreorderHours * 60 * 60 * 1000) {
        ...
        setErrorMessage(`Giỏ hàng có mẫu đặt trước, cần đặt trước ${formatPreorderLeadTime(requiredPreorderHours)}. Bạn có thể chọn thời gian xa hơn hoặc liên hệ shop để được hỗ trợ.`);
        return;
      }
    }
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      </> : <CatalogHeader query={query} setQuery={setQuery} goHome={goHome} setFiltersOpen={setFiltersOpen} filters={filters} setFilters={setFilters} sort={sort} setSort={setSort} visibleProducts={visibleProducts} resetFilters={resetFilters} onAdd={addToCart} categories={allCategories} tones={allTones} occasions={allOccasions} />}
...
function FilterBlock({ filters, setFilters, categories, tones, occasions }: { filters: Filters; setFilters: (filters: Filters) => void; categories: string[]; tones: string[]; occasions: string[] }) {
  return <div className="space-y-5"><div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Loại hoa</p><div className="flex flex-wrap gap-2 md:block md:space-y-1.5"><FilterPill active={filters.type === "all"} label="Tất cả" onClick={() => setFilters({ ...filters, type: "all" })} /><FilterPill active={filters.type === "bouquet"} label="Bó hoa" onClick={() => setFilters({ ...filters, type: "bouquet" })} /><FilterPill active={filters.type === "basket"} label="Giỏ hoa" onClick={() => setFilters({ ...filters, type: "basket" })} /></div></div>{categories.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Danh mục</p><select value={filters.category} onChange={(event) => setFilters({ ...filters, category: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả danh mục</option>{categories.map((category) => <option key={category}>{category}</option>)}</select></div>}{tones.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Tone màu</p><select value={filters.tone} onChange={(event) => setFilters({ ...filters, tone: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả màu</option>{tones.map((tone) => <option key={tone}>{tone}</option>)}</select></div>}{occasions.length > 0 && <div><p className="mb-2.5 text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Dịp tặng</p><select value={filters.occasion} onChange={(event) => setFilters({ ...filters, occasion: event.target.value })} className="h-11 w-full rounded-xl border border-border bg-surface px-3 text-sm outline-none focus:border-primary"><option value="all">Tất cả dịp</option>{occasions.map((occasion) => <option key={occasion}>{occasion}</option>)}</select></div>}<div><div className="mb-2.5 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-[.18em] text-muted-foreground">Giá tối đa</p><span className="text-xs font-bold text-primary">{formatVnd(filters.maxPrice)}</span></div><input aria-label="Giá tối đa" type="range" min="250000" max="1500000" step="50000" value={filters.maxPrice} onChange={(event) => setFilters({ ...filters, maxPrice: Number(event.target.value) })} className="w-full accent-[#71866c]" /></div></div>;
}
